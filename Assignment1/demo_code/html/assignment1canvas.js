...
function getWordAtLocation(aCanvasX, aCanvasY){
      ...
      let word_length = words[i].word.length;
		 if(Math.abs(words[i].x - aCanvasX) < (17*word_length) &&
...
function handleSubmitButton () {
	...
	if(userText && userText != ''){
      ...
      let textDiv = document.getElementById("text-area")// set text-area for incoming paragraph lines (this code get from w3Schools.com)
      var string = "";// var string will be presented for paragraph line

      if(responseObj.wordArray.length == 0){ // checking if data is incorrect
        document.getElementById('text-area').innerHTML = '';// clear paragraph lines
        words = []; // clear canvas as blank
      }

      let checking = responseObj.wordArray[0].y; // checking the y_coordinate to store individually each data' line (data is must be corrected and stored sucessfully)
      for(let i = 0; i < responseObj.wordArray.length;i++){ // loop through data wordArray to write paragraph in text-area field
        if(checking == responseObj.wordArray[i].y){ // checking incoming words, which are at the same line
          string = string + " " + responseObj.wordArray[i].word;//store incoming words in var string
        }
        else if (checking < responseObj.wordArray[i].y){//checking if incoming words, which are stared at the next line
            checking = responseObj.wordArray[i].y;// changing checking y_coordinate variable
            document.getElementById('text-area').innerHTML = document.getElementById('text-area').innerHTML + `<p> ${string} </p>`;// data line is showed in paragraph line
            string = ""; // reset string variable
            string = string + " " + responseObj.wordArray[i].word; // string add current word to prepare for restoring incoming words
        }
    }
