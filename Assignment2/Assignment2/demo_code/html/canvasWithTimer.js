...
var fontPointSize = 16; //point size for chord and lyric text
...
var pages_lines; // checking and re-new lines
...
function parseChordProFormat(chordProLinesArray){
  ...
  pages_lines = chordProLinesArray.length;
    ...
    for(var i=0; i<chordProLinesArray.length; i++) {
	 ...
	 if(lyricLine.trim().length > 0){
	   var theLyricWord = '';
     var the_word_contains_chord = '';
	   var theLyricLocationIndex = -1;
     var checking_chord_index = 0;
     var addtion_length;
     for(var j=0; j<lyricLine.length; j++){
        ...
        // read word containing chord if it is separating them
        //  or a single chord
        if(theLyricWord.includes('[')){
          addtion_length = theLyricWord.indexOf('[');
          the_word_contains_chord = theLyricWord.substring(0,theLyricWord.indexOf('['));
          the_word_contains_chord += theLyricWord.substring(theLyricWord.indexOf(']')+1,theLyricWord.length);
          // new lyricword does not include chord

          // chord is separated
          theLyricWord = theLyricWord.substring(theLyricWord.indexOf('[')+1,theLyricWord.indexOf(']'));
          checking_chord_index = checking_chord_index -1;
        }

        if(checking_chord_index == 0){
			   words.push({word: theLyricWord,
			               x: leftMargin + theLyricLocationIndex * characterWidth-40,
                           y: topMargin + i * 2 * lineHeight + lyricLineOffset,
						   lyric: 'Lyrics'
             });
           }
        else if  (checking_chord_index < 0){
          //add chord separated from word, or single chord
          words.push({word: theLyricWord,
 			               x: leftMargin + theLyricLocationIndex * characterWidth-26 + addtion_length*4,
                            y: topMargin + i *2* lineHeight + lyricLineOffset-20,
 						   chord: 'chords'
              });
            // add separated word from chord
          words.push({word: the_word_contains_chord,
     			            x: leftMargin + theLyricLocationIndex * characterWidth-40,
                                y: topMargin + i *2* lineHeight + lyricLineOffset,
     						   lyric: 'word'
                  });
                  the_word_contains_chord = '';
        }

			}
      checking_chord_index = 0;
		    theLyricWord = '';
...
//createa fuction for refreshbutton
function handleRefreshButton() {
  var line_index = 0; // checking index of lines;
  var stand_y = 65; // the minimum y_coodinate in canvas
  var standard_word = '';// Using this word to create a correct line (comapre it with others)
  let stand_line = [];// current line to add word;
  var new_pages = [''];// get new array to add lines;
  //run a while loop with every single lines in canvas1;
  console.log(words);
  while(line_index < pages_lines){

    //using a for loop to get the first word in each single line.
    for(let i =0; i < words.length;i++){
      if ((words[i].x == 0)&&(words[i].y >= stand_y - 30)&&(words[i].y <= stand_y))
        standard_word = words[i];
    }
      // using standard word to check other stay at the same line
      for(let i =0; i < words.length;i++){
          if  ((words[i].y >= standard_word.y-25)&&(words[i].y <= standard_word.y+15))
            stand_line.push(words[i]);// it is add the current word in line
      }

      stand_line.sort(function(a, b){return (a.x - b.x)}); // sort line by comparing word' x value
      new_pages[line_index] = stand_line; // add line into array;
      stand_line = []; // renew stand line;
      standard_word = ''; // renew stand word;
      line_index = line_index +1; // go in the next line;
      new_pages[line_index] = []; // create an index for new_pages array to avoid undefined
      stand_y = stand_y + 80; // get the next line' y coordinate
  }

  var returnfile = {array :new_pages};// set array value for reeturnfile
  var userRequestJSON = JSON.stringify(returnfile);
  // send  post json request to server
  $.post("refreshsong", userRequestJSON, function(data, status){
   console.log("data: " + data);
   console.log("typeof: " + typeof data);
   var responseObj = data;
   words =[]; // clear the current canvas;
   // re-new canvas, paragraph lines with new-values;
   if(responseObj.songLines) parseChordProFormat(responseObj.songLines);
    });
}
...
if(userText && userText !== ''){
   var userRequestObj = {text: userText};
     var userRequestJSON = JSON.stringify(userRequestObj);
   $('#userTextField').val(''); //clear the user text field
    // do the same thing as handle refresh but we have user input in this case
    // reading canvas lines, sorting
    var line_index = 0;
    var stand_y = 65;
    var standard_word = '';
    let stand_line = [];
    ...
    while(line_index < pages_lines){
      ...
      for(let i =0; i < words.length;i++){
        ...
        for(let i =0; i < words.length;i++){
            if  ((words[i].y >= standard_word.y-25)&&(words[i].y <= standard_word.y+15))
              stand_line.push(words[i]);
        }
        stand_line.sort(function(a, b){return (a.x - b.x)});
        new_pages[line_index] = stand_line;
        stand_line = [];
        standard_word = '';
        line_index = line_index +1;
        new_pages[line_index] = [];
        stand_y = stand_y + 80;
    }
    // set returnfile with new_pages, and users input to create a song txt file.
    var returnfile = {array :new_pages, name: userRequestObj.text};

    var userRequestJSON = JSON.stringify(returnfile);
    // send post to client with array of lines to write a new song.
    $.post("write_file", userRequestJSON, function(data, status){
     console.log("data: " + data);
     console.log("typeof: " + typeof data);});
  }
}
